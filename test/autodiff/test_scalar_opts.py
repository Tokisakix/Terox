...
from terox.autodiff.scalar_opts import Add, Sub, Mul, Div, Inv, Neg, Max, Min, Eq, Lt, Gt, Abs, Exp, Log, Relu, Sigmoid, Tanh
...
def test_inv(a:float=2.0) -> None:
    A = Scalar(a, None, None)
    C = scalar_opts_backend.Inv(A)
    assert 1.0 / A.item() == C.item()
    assert C._history._func.__class__ == Inv
